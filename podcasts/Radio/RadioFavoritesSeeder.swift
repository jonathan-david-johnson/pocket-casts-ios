...
enum RadioFavoritesSeeder {
    ...
    /// `Constants.UserDefaults.radioFavoritesSeededM7` — sets the flag only
    ...
    static func seedIfNeeded() {
        ...
        guard !defaults.bool(forKey: Constants.UserDefaults.radioFavoritesSeededM7) else { return }
        ...
        guard !curated.isEmpty else {
            defaults.set(true, forKey: Constants.UserDefaults.radioFavoritesSeededM7)
            return
        }
        ...
        Task { @MainActor in
            ...
            for station in curated {
                do {
                    try await RadioFavoritesManager.shared.addFavorite(stationId: station.defaultSeedUUID)
                } catch {
                    allSucceeded = false
